...
def get_messages_by_customer(
    customer_id: str = Path(..., description="Customer ID as string"),
    db: Session = Depends(get_db),
):
    messages = message_service.get_messages_by_customer_uuid(db, customer_id)
    if not messages:
...
def list_messages(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return message_service.get_all_messages(db, skip=skip, limit=limit)
...
def get_user_messages(wa_id: str, db: Session = Depends(get_db)):
    messages = message_service.get_messages_by_wa_id(db, wa_id)
    if not messages:
